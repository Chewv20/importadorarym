...
/** @var ?array $logo */
$l = $logo;
$action = $l ? url('/admin/logos-proveedores/' . (int) $l['id']) : url('/admin/logos-proveedores');
?>
<div class="admin-head"><h1><?= $l ? 'Editar' : 'Nuevo' ?> logo de proveedor</h1></div>
...
<form class="form form--admin" method="post" action="<?= $action ?>" enctype="multipart/form-data" novalidate>
    ...
    <div class="form-actions">
        ...
        <a class="btn btn--outline" href="<?= url('/admin/logos-proveedores') ?>">Cancelar</a>
    </div>
</form>
